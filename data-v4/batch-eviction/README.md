...
- [`batch-completion-index.csv`](batch-completion-index.csv) contains one
  ...
  runs.
- [`results.html`](results.html) provides the visual report, four matched
  scenarios, data dictionary, filtering, and CSV export.
- [`run-config.json`](run-config.json) records the fixed traffic, vLLM,
  Endpoint Picker, and Async Processor settings.

## Scope

This evidence covers one model replica. It demonstrates the tested
single-replica eviction and retry path; multi-replica coordination and a fixed
service-level objective require separate tests.
